  ...
  persist(
    (set, get) => ({
      ...
      addBusiness: (name, type, stage = 'idea') => {
        ...
        const newBiz: Business = { id, name, type, stage, progress_step: 0, status: 'setup', stepData: {} }
      ...
      updateStepData: (businessId, section, data) => {
        set(state => ({
          businesses: state.businesses.map(b => 
            b.id === businessId 
              ? { ...b, stepData: { ...b.stepData, [section]: { ...(b.stepData[section] || {}), ...data } } }
              : b
          )
        }))
      },
      ...
      setProgress: (businessId, step) => {
        set(state => ({
          businesses: state.businesses.map(b => b.id === businessId ? { ...b, progress_step: step } : b)
        }))
      },
      
      deleteBusiness: (businessId) => {
        set(state => ({ businesses: state.businesses.filter(b => b.id !== businessId) }))
      },
